...
function DriverPanel({ driverId, onClose }: { driverId: string; onClose: () => void }) {
  ...
  const contrat  = contratsConducteurs.find(c => c.chauffeurId === driverId);
  const paie     = paieMensuelle.find(p => p.chauffeurId === driverId);
  const driverFormations = formations.filter(f => f.chauffeurId === driverId);
  const driverConges     = conges.filter(c => c.chauffeurId === driverId);
      ...
      <div className="glass-card w-full max-w-lg h-full overflow-y-auto p-6"
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: '#4a7a9b' }}>{driver.matricule}</p>
              </div>
            </div>
        ...
        <div className="px-4 py-4 rounded-xl mb-5"
          ...
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm" style={{ color: '#7bacc8' }}>Score sécurité</span>
          ...
          <div className="grid grid-cols-4 gap-2 mt-3">
            {[
              ...
              const c = v >= 80 ? '#00e676' : v >= 65 ? '#ffb300' : '#ff4444';
              return (
        ...
        {contrat && (
          <div className="mb-5">
            <h4 className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#4a7a9b' }}>Contrat</h4>
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'Type contrat',   value: contrat.type,                                        color: '#e8f4fd' },
                { label: 'Ancienneté',     value: `${contrat.anciennete} an(s)`,                       color: '#e8f4fd' },
                { label: 'Salaire base',   value: `${contrat.salaireBase.toLocaleString()} MAD`,       color: '#00d4ff' },
                { label: 'Prime km',       value: `${contrat.primeKm} MAD/km`,                         color: '#00e676' },
                { label: 'Date embauche',  value: contrat.dateEmbauche,                                color: '#7bacc8' },
        ...
        {/* Documents */}
        <div className="mb-5">
          <h4 className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: '#4a7a9b' }}>Documents</h4>
            ...
            {[
              ...
              return (
                <div key={label} className="flex items-center justify-between px-3 py-2.5 rounded-lg"
                  ...
                  <span className="text-xs" style={{ color: '#7bacc8' }}>{label}</span>
        ...
        {paie && (
          <div className="mb-5">
            ...
            <div className="space-y-1.5">
              {[
                { label: 'Salaire de base',   value: paie.salaireBase,      color: '#7bacc8' },
                { label: 'Prime km',          value: paie.primeKm,          color: '#00e676' },
                { label: 'Prime rendement',   value: paie.primeRendement,   color: '#00e676' },
                { label: 'Heures supp.',      value: paie.heuresSupp,       color: '#00d4ff' },
              ...
              <div className="flex justify-between text-sm px-3 py-2 rounded-lg font-bold mt-1"
                ...
                <span style={{ color: '#7bacc8' }}>NET À PAYER</span>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg mb-4"
          ...
          <Phone size={14} style={{ color: '#4a7a9b' }} />
          <span className="text-xs" style={{ color: '#7bacc8' }}>{driver.phone}</span>
        </div>
              ...
              {driverFormations.map(f => (
                <div key={f.id} className="px-3 py-2.5 rounded-lg"
                  ...
                  <div className="flex justify-between items-start">
                    <p className="text-xs font-medium" style={{ color: '#e8f4fd' }}>{f.intitule}</p>
                    {f.certificat && (
                      <Award size={12} style={{ color: '#ffb300', flexShrink: 0, marginLeft: 4 }} />
                    )}
                  </div>
                  <p className="text-xs mt-0.5" style={{ color: '#4a7a9b' }}>
              ...
              {driverConges.map(c => (
                <div key={c.id} className="px-3 py-2.5 rounded-lg"
                  ...
                  <div className="flex items-center justify-between mb-0.5">
                    <Badge label={congeTypeLabel[c.type]} className={congeTypeColor[c.type]} />
                    <Badge label={c.statut} className={congeStatutColor[c.statut]} />
                  </div>
                  <p className="text-xs" style={{ color: '#7bacc8' }}>
                    {c.dateDebut} → {c.dateFin} · <strong>{c.jours} jour(s)</strong>
                  </p>
                  {c.motif && <p className="text-xs mt-0.5 italic" style={{ color: '#4a7a9b' }}>{c.motif}</p>}
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard label="Documents à renouveler" value={docsExpirant}
            icon={FileText} iconColor="#ff4444" iconBg="rgba(255,68,68,0.1)"
          ...
          <KPICard label="Masse salariale (Mai)" value={`${(masseSalariale/1000).toFixed(0)}K`} unit="MAD"
            icon={DollarSign} iconColor="#00d4ff" iconBg="rgba(0,212,255,0.1)"
          ...
          <div className="space-y-4">
            ...
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-3" style={{ borderBottom: '1px solid #1e3a5f' }}>
                <span className="text-sm font-semibold" style={{ color: '#e8f4fd' }}>Liste des conducteurs</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {drivers.map(d => {
                      const contrat      = contratsConducteurs.find(c => c.chauffeurId === d.id);
                        ...
                        <tr key={d.id} className="table-row-hover cursor-pointer"
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="text-sm font-medium" style={{ color: '#e8f4fd' }}>
                                  {d.prenom} {d.nom}
                                </div>
                                <div className="text-xs" style={{ color: '#4a7a9b' }}>{d.matricule}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-12 h-1.5 rounded-full" style={{ background: '#1e3a5f' }}>
                                <div className="h-full rounded-full"
                          ...
                          <td className="px-4 py-3 text-sm font-mono" style={{ color: '#7bacc8' }}>
        ...
        {activeTab === 'conges' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Approuvés',    count: conges.filter(c=>c.statut==='approuve').length,   color: '#00e676' },
                { label: 'En attente',   count: conges.filter(c=>c.statut==='en_attente').length,  color: '#ffb300' },
                { label: 'Refusés',      count: conges.filter(c=>c.statut==='refuse').length,      color: '#ff4444' },
            ...
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-3" style={{ borderBottom: '1px solid #1e3a5f' }}>
                <span className="text-sm font-semibold" style={{ color: '#e8f4fd' }}>Demandes de congés</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                  ...
                  <tbody>
                    {conges.map(c => {
                      const d = drivers.find(x => x.id === c.chauffeurId);
                      return (
                        <tr key={c.id} className="table-row-hover" style={{ borderBottom: '1px solid #1e3a5f26' }}>
                          <td className="px-4 py-3">
                            <div className="text-sm font-medium" style={{ color: '#e8f4fd' }}>
                              {d ? `${d.prenom} ${d.nom}` : '—'}
                            </div>
                          ...
                          <td className="px-4 py-3 text-sm font-bold" style={{ color: '#e8f4fd' }}>
                            {c.jours}j
                          </td>
                          <td className="px-4 py-3 text-xs italic" style={{ color: '#4a7a9b' }}>
                            {c.motif || '—'}
                          </td>
                          <td className="px-4 py-3">
                            <Badge label={c.statut} className={congeStatutColor[c.statut]} />
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {drivers.map(d => {
                ...
                return (
                  <div key={d.id} className="glass-card p-4 cursor-pointer hover:border-[#234878] transition-all"
                    onClick={() => setSelectedDriver(d.id)}>
                    ...
                    <div className="flex justify-between text-xs mb-3">
                      <span style={{ color: '#4a7a9b' }}>Formations</span>
                      <span className="font-bold" style={{ color: '#00d4ff' }}>{driverForms.length}</span>
                    </div>
                    <div className="flex justify-between text-xs mb-3">
                      <span style={{ color: '#4a7a9b' }}>Certifiées</span>
                    ...
                    {driverForms.length === 0 && (
                      <p className="text-xs italic" style={{ color: '#2a5070' }}>Aucune formation enregistrée</p>
                    )}
                    {driverForms.slice(0, 2).map(f => (
                      <div key={f.id} className="text-xs mb-1 truncate" style={{ color: '#4a7a9b' }}>
                        · {f.intitule}
                      </div>
                    ))}
                    <div className="flex items-center justify-end mt-2 text-xs" style={{ color: '#4a7a9b' }}>
                  ...
                  <tbody>
                    {formations.map(f => {
                      ...
                      const expJours = f.expiration
                        ...
                        : null;
                      const expClr = expJours === null ? '#4a7a9b' : expJours < 0 ? '#ff4444' : expJours < 180 ? '#ffb300' : '#00e676';
                      return (
                        <tr key={f.id} className="table-row-hover" style={{ borderBottom: '1px solid #1e3a5f26' }}>
                          ...
                          <td className="px-4 py-3 text-sm" style={{ color: '#7bacc8' }}>{f.intitule}</td>
                          <td className="px-4 py-3 text-xs" style={{ color: '#4a7a9b' }}>{f.organisme}</td>
                          <td className="px-4 py-3 text-xs" style={{ color: '#7bacc8' }}>{f.date}</td>
                          <td className="px-4 py-3 text-xs" style={{ color: '#7bacc8' }}>{f.dureeJours}j</td>
                          <td className="px-4 py-3">
                            {f.certificat
                              ? <span className="flex items-center gap-1 text-xs" style={{ color: '#ffb300' }}><Award size={12} /> Oui</span>
                              : <span className="text-xs" style={{ color: '#2a5070' }}>Non</span>}
                          </td>
        ...
        {activeTab === 'paie' && (
          <div className="space-y-4">
            ...
            <div className="glass-card p-5">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={paieChartData} margin={{ top: 5, right: 10, left: -5, bottom: 30 }}>
                  ...
                  <XAxis dataKey="name" tick={{ fill: '#4a7a9b', fontSize: 10, angle: -20, textAnchor: 'end' }}
            ...
            <div className="glass-card overflow-hidden">
              <div className="px-5 py-3" style={{ borderBottom: '1px solid #1e3a5f' }}>
                <span className="text-sm font-semibold" style={{ color: '#e8f4fd' }}>Bulletin de paie — Mai 2025</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                  ...
                  <tbody>
                    {paieMensuelle.map(p => {
                      const d = drivers.find(x => x.id === p.chauffeurId);
                      return (
                        <tr key={p.chauffeurId} className="table-row-hover" style={{ borderBottom: '1px solid #1e3a5f26' }}>
                          <td className="px-4 py-3">
                            <div className="text-sm font-medium" style={{ color: '#e8f4fd' }}>
                              {d ? `${d.prenom} ${d.nom}` : '—'}
                            </div>
                          ...
                          <td className="px-4 py-3 text-sm" style={{ color: '#7bacc8' }}>{p.salaireBase.toLocaleString()}</td>
                          ...
                          <td className="px-4 py-3 text-sm" style={{ color: '#00d4ff' }}>{p.heuresSupp.toLocaleString()}</td>
              ...
              <div className="px-5 py-3 flex justify-between items-center"
                ...
                <div className="flex gap-6">
                  {[
                    { label: 'Total brut', value: paieMensuelle.reduce((s,p)=>s+p.salaireBase+p.primeKm+p.primeRendement+p.heuresSupp, 0), color: '#7bacc8' },
                ...
                <div className="text-right">
                  <div className="text-xs" style={{ color: '#4a7a9b' }}>Masse salariale nette</div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Salaires de base',   value: paieMensuelle.reduce((s,p)=>s+p.salaireBase,0),                              pct: 0, color: '#00d4ff' },
                { label: 'Primes & HS',        value: paieMensuelle.reduce((s,p)=>s+p.primeKm+p.primeRendement+p.heuresSupp,0),    pct: 0, color: '#00e676' },
                { label: 'Retenues CNSS/IR',   value: paieMensuelle.reduce((s,p)=>s+p.retenues,0),                                  pct: 0, color: '#ff4444' },
              ].map(item => {
                ...
                return (
                  <div key={item.label} className="glass-card p-4">
                    ...
                    <div className="text-xs mb-2" style={{ color: '#4a7a9b' }}>{item.label}</div>
